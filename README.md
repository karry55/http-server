...
- V4 是把 V3 的线程数从 4 改成 8 后重跑的结果
- V5 与 V3 线程数完全相同，唯一差异是多了一层 Redis 缓存
- V6 与 V5 的差异只有路由和缓存粒度
- V7 与 V6 的差异只有"是否循环读到 `EAGAIN`"
...
| V6 RESTful 版 | `http_server_restful.cpp` | 4 | **32670** | **2.67 ms** | —— | 213.88 ms | —— | 0 |
| V7 EPOLLET 循环读版 | `http_server_et_loop.cpp` | 4 | 26821 | 5.53 ms | —— | 833.38 ms | —— | 0 |
...
**V6 RESTful 版（4 线程）—— 补全接口，性能零代价。**
V6 在 V5 基础上把接口补齐成完整的 REST 语义：新增 `GET /todo/<id>` 和 `PUT /todo/<id>`，
缓存也从"整表一个 key"扩展成"整表 + 逐条（`todo_<id>`）"两级。

压测结果 **32670 QPS / Avg 2.67 ms / Max 213.88 ms，与 V5 完全一致**。

这个"完全一致"本身就是结论：压测负载打的是同一批端点，**新增的路由和更细的缓存粒度
没有带来任何可测量的开销**。V5 → V6 是一次纯功能扩展，不需要在性能上做任何权衡。

**V7 EPOLLET 循环读版（4 线程）—— 用一点吞吐换正确性。**
V7 修掉了前几个版本的 ET 缺陷：`EPOLLET` 下必须循环 `read` 到 `EAGAIN`，
否则请求报文被 TCP 拆成多段时只能拿到第一段，请求体超过 4096 字节会被静默截断。

- QPS 32670 → 26821，下降约 **18%**
- Avg 延迟 2.67 ms → 5.53 ms
- Max 延迟 213.88 ms → 833.38 ms
- 错误数仍然是 0

代价来自"循环读"这一步：每个请求至少多一次 `read` 系统调用（要读到返回 `EAGAIN` 才退出循环），
而且读循环是在**事件循环线程**里跑的，会占用一点事件循环的时间，延迟的波动也变大了
（Max 从 214 ms 涨到 833 ms）。

> 这份数据只说明"循环读带来了开销"，**没有隔离出具体是哪一项占主导**。
> 要搞清楚的话，需要把 V7 拆成"只加循环读"和"循环读 + 完整 CRUD"两个变体分别压测。

**但这个代价必须付。** V6 那 32670 QPS 有一部分是"假的"——大于 4096 字节的请求会被截断，
而压测里所有请求都很小，所以跑不出错；真实的大请求会拿到错误的内容。
**V7 的 26821 QPS 才是能正确处理任意大小请求体的真实吞吐。**

如果两个都想要，可以只在"报文明显没收全"时才继续读——例如先检查是否已经读到 `\r\n\r\n`、
长度是否满足 `Content-Length`，凑齐了就立刻交付，不必每次都读到 `EAGAIN`。
那样小请求就能保持 V6 的开销。
...
| V5 → V6 | 接口不完整、缓存粒度太粗 | 补全 CRUD，加逐条缓存 | 功能对齐 REST 语义，**性能零损失** |
| V6 → V7 | ET 模式下不循环读会截断报文 | 循环读到 `EAGAIN` | 大请求体不再出错，吞吐降约 18% |

### 测试口径与局限

- 工具 `wrk`，环境 WSL，被测对象是 V1–V7
- **本轮没有记录具体的接口配比**（打了哪些路径、各占多少比例），所以上面这组数字
  适合看横向趋势，但无法独立复现
- V6 与 V5 的数字完全相同，稳妥起见应确认这是两次独立测量，而不是同一次结果被复用了两次
